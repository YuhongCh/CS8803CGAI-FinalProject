...
public:
    ...
    inline typename std::vector<T>::const_reference operator()(const Vector2i& index) const {
        return m_container[index.x() * m_dimension[1] + index.y()];
    }
    inline typename std::vector<T>::reference operator()(const Integer& xi, const Integer& yi) {
        return m_container[xi * m_dimension[1] + yi];
    }
    inline typename std::vector<T>::reference operator()(const Vector2i& index) {
        return m_container[index.x() * m_dimension[1] + index.y()];
    }
...
public:
    ...
    inline typename std::vector<T>::const_reference operator()(const Vector3i& index) const {
        return m_container[(index.x() * m_dimension.y() + index.y()) * m_dimension.z() + index.z()];
    }
    inline typename std::vector<T>::reference operator()(const Integer& xi, const Integer& yi, const Integer& zi) {
        return m_container[(xi * m_dimension.y() + yi) * m_dimension.z() + zi];
    }
    inline typename std::vector<T>::reference operator()(const Vector3i& index) {
        return m_container[(index.x() * m_dimension.y() + index.y()) * m_dimension.z() + index.z()];
    }
